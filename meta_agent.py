...
class MetaAgent(AgentSystem):
    def forward(self, repo_path, eval_path, iterations_left=None, parent_genid=None):
        ...
        instruction = (
            ...
            "which generation(s) it's about, links to related notes -- see "
            "memory_append's own description for the exact schema) and immutable "
            "-- write a new note instead of trying to edit an old one. Before "
            "writing a note, consider using memory_search with a draft of it "
            "first to see what's already related; its results are candidates "
            "for that note's own links field, not something to link automatically. "
            "memory_search also works as plain ad-hoc lookup any time you want "
            "notes about something specific instead of reading everything.\n\n"
