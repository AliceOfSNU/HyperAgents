...
def _meta_agent_env_vars():
    ...
    task-agent/evaluator-agent containers: still visible to a sufficiently
...
def run_harness_research(root_dir, output_dir, genid, run_eval=True):
    # NOTE: like polyglot, research has its own harness because every
    # task/evaluator agent run must happen inside a fresh sandboxed
    # container (see domains/research/harness.py) rather than through the
    # generic per-domain CSV harness.
    from domains.research.harness import run_harness_research as _run_harness_research

    return _run_harness_research(root_dir, output_dir, genid, run_eval=run_eval)
...
def generate_loop(
    ...
    try:
        for current_genid in range(start_genid, max_generation + 1):
            ...
            # Evaluate the agent on research if needed. Unlike polyglot,
            # run_eval=False doesn't skip the whole call -- it still needs to
            # run so maybe_run_evaluator_promotion's own checkpoint schedule
            # (every EPOCH_LENGTH_GENERATIONS gens, independent of any single
            # generation's diff) doesn't silently slip. See
            # domains/research/harness.py's run_harness_research docstring.
            if "research" in domains:
                run_harness_research(root_dir, output_dir, current_genid, run_eval=metadata["run_eval"])
