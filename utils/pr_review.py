...
def run_pr_review_gate(root_dir, run_id, genid, parent_branch, raw_patch_content, remote="fork", poll_interval=30, logging=print):
    ...
    INSTRUCTIONS.md) can end up staged at the repo root if something runs
    task_agent.py without isolating its cwd first, and `git apply` can't
    handle a diff full of binary content anyway -- so this makes sure it's
    never treated as a real self-modification even if it somehow ends up
    tracked. (Confirmed against a real run: the meta-agent's now-removed
    consult_task_agent tool did exactly this before it was deleted.)
    ...
    """
    patch_content = filter_patch_by_files(raw_patch_content, ["domains/", "workspace/"])
