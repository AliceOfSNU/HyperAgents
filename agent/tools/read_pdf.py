...
Use this for PDFs sitting in the steering directory (or anywhere else in the
container) -- e.g. arxiv preprints a human overseeing this run has dropped in
for you to optionally read. Plain text/markdown files can just be read with
the bash tool (cat/sed); this tool exists because PDF is a binary format.

* `path` must be a local file path (e.g. one you found via `ls` on the
  steering directory), not a URL -- use fetch_url first if you only have a link.
