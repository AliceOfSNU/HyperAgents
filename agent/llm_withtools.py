    ...
    try:
        ...
        while tool_calls:
            ...
            for call in calls_to_run:
                ...
                display_labels.append(f"{tool_name}({tool_input})")
            ...
            for call in tool_calls:
                if call["id"] not in executed_ids:
                    ...
                    display_labels.append(f"{call['name']} (skipped)")
            ...
            # Logged only now, after the budget suffix is appended, so the
            # persisted transcript shows exactly what the model was actually
            # given -- previously this logged the raw tool_output before that
            # append, so the countdown reaching the model was invisible in
            # meta_agent_chat_history.md (confirmed live: a transcript
            # inspection couldn't tell whether the notice had been sent at
            # all, even though it was correctly part of the real API payload).
            for label, result_msg in zip(display_labels, tool_result_msgs):
                logging(f"Tool output: {label} -> {repr(result_msg['text'])}")
