...
`score_report_text_items` takes a pluggable `score_fn`, so the same
weight-aggregation logic scores both real_score (via score_item_with_claude)
and the evaluator_agent's own per-item output.
"""
...
# `anthropic` is imported lazily inside score_item_with_claude, not here --
# this module is also imported inside the sandboxed containers (for
# score_report_text_items, used by run_evaluator_agent.py), which don't have
# the anthropic package installed since only host-side real_score needs it.
