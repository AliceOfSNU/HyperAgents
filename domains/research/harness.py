...
Every execution of meta-agent-authored code -- task_agent.py, which uses
bash/editor tools, i.e. runs arbitrary shell commands -- happens inside a
fresh, disposable container built from the same "hyperagents" image the
meta-agent itself runs in. Nothing from this file ever runs meta-agent code
on the host.

Node utility per generation: run the candidate task_agent.py against a fixed
5-10 task "scoring subset", score each resulting report with real_score
(Claude via the real anthropic SDK, domains/research/claude_scorer.py --
trusted, non-evolvable, safe to run on the host); node utility is the mean
real_score over the scoring subset.
"""
...
from .claude_scorer import score_item_with_claude
from .scoring_utils import score_report_text_items
from .config import (
    CLAUDE_JUDGE_MODEL,
    RCB_ROOT,
    RCB_TASKS_DIR,
...
AGENT_MODEL = "deepseek/deepseek-v4-flash"  # task_agent backend during search
...
# NOT claude_scorer.py: it holds the actual ground-truth rubric/prompt/
# Claude-calling implementation, and it's never needed inside a task-agent
# container -- scoring happens host-side, in compute_node_utility below.
BASELINE_FILES = [
    "task_agent.py",
    "run_task_agent.py",
...
def _log_container_output(exec_result):
    ...
    about them ourselves, not have the whole task-agent run abort here."""
...
def _process_scoring_task(root_dir, patch_files, task_id, model, gen_eval_dir, log_path, stagger_s=0):
    ...
    try:
        ...
        if not ok:
            return {
                "task_id": task_id, "real_score": 0.0,
                "task_agent_ok": False, "real_items": [], "report_empty": True,
            }

        report_text = read_report(local_out_dir) or ""
        if not report_text.strip():
            # Nothing worth scoring -- an empty report would just make Claude
            # grade every checklist item ~0 anyway, so skip the LLM-scoring
            # call rather than spending real API cost on a foregone
            # conclusion. report_empty is also what compute_node_utility's
            # gate checks to decide whether to bail out of the rest of the
            # scoring subset early.
            return {
                "task_id": task_id, "real_score": 0.0,
                "task_agent_ok": True, "real_items": [], "report_empty": True,
            }
        ...
        return {
            "task_id": task_id, "real_score": real_score, "task_agent_ok": True,
            "real_items": real_items, "report_empty": False,
        }
    finally:
        docker_client.close()
...
def compute_node_utility(docker_client, root_dir, output_dir, genid, model=AGENT_MODEL, max_workers=MAX_WORKERS):
    ...
    real_score, and return (node_utility, per_task_records).
    ...
    burning a full task-agent run on each of them for a foregone conclusion.
    The remaining tasks only run if at least one gate task actually produced
    a report.
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(
                    ...
                    task_id, model, gen_eval_dir, log_path,
    ...
    if all_gate_empty:
        ...
        skipped_records = [
            {
                "task_id": task_id, "real_score": 0.0,
                "task_agent_ok": False, "real_items": [],
    ...
    node_utility = sum(r["real_score"] for r in per_task_records) / len(per_task_records) if per_task_records else 0.0
...
def run_harness_research(root_dir, output_dir, genid):
    ...
    re-scoring would be pure waste."""
    log_path = Path(output_dir) / f"gen_{genid}" / "research_eval" / "generate.log"
